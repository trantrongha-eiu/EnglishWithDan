...
mongoose.connect(process.env.MONGO_URI)
  .then(async () => {
    ...
    try {
      ...
      logger.startup('ListeningTip seeding...');
      await runListeningTipsSeed();
      logger.startup('ListeningTip seed done');
    } catch (e) {
    ...
    // Start stale in-progress attempt timeout-sweep cron
    try {
      attemptTimeoutSweepCron.start();
    } catch (e) {
    ...
    // Start class-attendance daily recompute sweep
    try {
      classAttendanceSweepCron.start();
    } catch (e) {
    ...
    // Retry Speaking answers whose AI grading failed at submit (overload)
    try {
      speakingGradeQueueCron.start();
    } catch (e) {
