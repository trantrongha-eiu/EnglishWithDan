...
export default function ListeningSectionEdit() {
  ...
  // Gap-fill: separate state since it's reviewed/saved through its own
  // endpoint (PUT .../gapfill), not the main section autosave — an admin
  // editing the transcript shouldn't accidentally publish stale gap-fill
  // content, and vice versa.
  const [gapFill, setGapFill] = useState({ template: '', answers: [], published: false, generatedAt: null });
  const [gfGenerating, setGfGenerating] = useState(false);
  ...
  useEffect(() => {
    ...
    apiFetch(`/admin/listening/sections/${savedId}`)
      .then(d => {
        ...
        setGapFill({
          template: s.gapFillTemplate || '',
          answers: s.gapFillAnswers || [],
          published: s.gapFillPublished === true,
          generatedAt: s.gapFillGeneratedAt || null,
        });
      })
  ...
  async function generateGapFill() {
    if (!savedId) { toast('Lưu section trước khi sinh Gap-fill', 'error'); return; }
    if (!meta.transcript.trim()) { toast('Section chưa có transcript', 'error'); return; }
    setGfGenerating(true);
    try {
      const d = await apiFetch(`/admin/listening/sections/${savedId}/gapfill/generate`, { method: 'POST' });
      setGapFill({
    ...
    finally { setGfGenerating(false); }
  }

  async function saveGapFill(publish) {
    setGfSaving(true);
    try {
      const payload = { gapFillTemplate: gapFill.template, gapFillAnswers: gapFill.answers, gapFillPublished: publish };
      await apiFetch(`/admin/listening/sections/${savedId}/gapfill`, { method: 'PUT', body: JSON.stringify(payload) });
      setGapFill(f => ({ ...f, published: publish }));
      toast(publish ? 'Đã publish Gap-fill ✓' : 'Đã lưu Gap-fill ✓');
    } catch (err) { toast(err.message, 'error'); }
    finally { setGfSaving(false); }
  }

  function setGapFillAnswerAt(idx, value) {
    setGapFill(f => {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '24px 20px' }}>
      ...
      {/* Gap-fill (AI-generated, requires admin review before publish) */}
      {savedId && (
        <div className="card" style={{ padding: 20, marginBottom: 20 }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={generateGapFill} disabled={gfGenerating || !meta.transcript.trim()}>
            {gfGenerating ? '⏳ Đang sinh bằng AI...' : gapFill.template ? '🪄 Sinh lại bằng AI' : '🪄 Sinh Gap-fill bằng AI'}
          </button>
          ...
          {gapFill.template && (
            <>
                  ...
                  {gapFill.answers.map((a, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <input className="form-input" style={{ fontSize: 12 }} value={a}
                        onChange={e => setGapFillAnswerAt(i, e.target.value)} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                <button className="btn btn-ghost" onClick={() => saveGapFill(false)} disabled={gfSaving}>
                  💾 Lưu (chưa publish)
                </button>
                <button className="btn btn-primary" onClick={() => saveGapFill(true)} disabled={gfSaving}>
                  {gfSaving ? 'Đang lưu...' : gapFill.published ? '✅ Đã publish (lưu lại)' : '🚀 Publish cho học sinh'}
                </button>
                {gapFill.published && (
                  <button className="btn btn-ghost" onClick={() => saveGapFill(false)} disabled={gfSaving}>
                    ⏸ Gỡ publish
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <div style={{ marginBottom: 16, padding: '10px 14px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12, display: 'flex', flexWrap: 'wrap', gap: 14, alignItems: 'center' }}>
          ...
          {(() => {
            ...
            const color = totalQs === expected ? '#16a34a' : totalQs > 0 ? '#d97706' : 'var(--danger)';
