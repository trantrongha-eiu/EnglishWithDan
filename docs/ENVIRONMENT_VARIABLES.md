...
| Variable | Required? | Purpose | Used in | If missing |
|---|---|---|---|---|
| `MEDIA_TOKEN_SECRET` | **Required in production** (optional in dev/test) | Server-only secret that seals/verifies the short-lived media tokens which replace permanent Cloudinary URLs (AES-256-GCM). Deliberately **separate from `JWT_SECRET`** — leaking one must not compromise the other. Used identically for both sealing and verifying (one `key()` in `backend/services/mediaTokenService.js`), so there is never a two-secret mismatch. | `backend/config/index.js` (`media.tokenSecret`), `backend/services/mediaTokenService.js`, `backend/server.js` (startup guard) | **Production:** `backend/server.js` refuses to start (`logger.error('startup', 'MEDIA_TOKEN_SECRET is not set …')` → `process.exit(1)`) — it does **not** silently fall back to `JWT_SECRET`. **Dev/test (`NODE_ENV !== 'production'`):** falls back to `JWT_SECRET` so nothing new needs setting locally. Generate with `node -e "console.log(require('crypto').randomBytes(48).toString('hex'))"`. |
...
| `GROQ_API_KEY` | Optional | Groq — **last-resort** fallback Speaking grading engine (transcript-only), tried after Gemini and Mistral both fail. Also the Gemini fallback for sample-answer / hints / improve. | `backend/services/groqService.js`, `backend/services/speakingService.js`, `backend/config/index.js` | Speaking grading surfaces Gemini's original error on failure instead of retrying via Groq — no other feature affected. |
| `MISTRAL_API_KEY` | Optional | Mistral — **2nd** Speaking-grading fallback (tried before Groq). Voxtral is audio-native, so on this tier Pronunciation can still be graded from the recording. Free "Experiment" tier on La Plateforme; unset = never called, no cost. | `backend/services/mistralService.js`, `backend/services/speakingService.js` (`gradeSpeaking` → `_gradeSpeakingFallback`), `backend/config/index.js` | Speaking grading skips straight from Gemini to Groq (transcript-only) on a Gemini failure. Nothing else affected. Verify once against a live key — the exact Voxtral audio content-part shape isn't exercised by any test. |
