...
5. **If you changed any `frontend/js/*.js` or `frontend/css/*.css` file (or
   ...
   `?v=YYYYMMDD` query string (current version: `20260826` — e.g.
   `js/dashboard.js?v=20260826`) — Render
   ...
   Get-ChildItem frontend/*.html | ForEach-Object {
     (Get-Content $_.FullName -Raw) -replace '\?v=20260826', '?v=20260827' | Set-Content $_.FullName -NoNewline
   }
   ```
