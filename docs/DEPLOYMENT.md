...
5. **If you changed any `frontend/js/*.js` or `frontend/css/*.css` file (or
   ...
   `?v=YYYYMMDD` query string (current version: `20260831` — e.g.
   `js/dashboard.js?v=20260831`) — Render
   ...
   Get-ChildItem frontend/*.html | ForEach-Object {
     (Get-Content $_.FullName -Raw) -replace '\?v=20260831', '?v=20260901' | Set-Content $_.FullName -NoNewline
   }
   ```
