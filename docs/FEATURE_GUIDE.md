...
3. **`teacherOnly` vs `adminOnly`** — based on the real, consistent split across every existing admin route file:
   ...
   - **`adminOnly`** (admin role only) — anything destructive, financial, or role/permission-changing: user account creation/edit/ban/delete and role changes (`backend/routes/admin/users.js`), premium plan changes and upgrade-request approval (`backend/routes/admin/premium.js`), `db-status` and one-off maintenance/reseed endpoints (`backend/routes/admin/stats.js`, `task2Topics.js`). If your new route mutates money, roles, or permanently destroys data, default to `adminOnly`.
